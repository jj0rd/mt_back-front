...
public class RecommendationService {
    ...
    public RecommendationService(RestTemplate restTemplate, FeatureExtractionService featureExtractionService,
                                 KeywordService keywordService, PersonService personService) {
    ...
    public Map<String, Object> discoverFuzzy(MovieFuzzyDiscoverRequest request) {
        ...
        if (request.getPeople() != null && !request.getPeople().isEmpty()) {
            for (Integer personId : request.getPeople()) {

                String urlC = tmdbApiUrl + "/person/" + personId
                        + "/movie_credits?api_key=" + tmdbApiKey;

                Map<String, Object> result =
                        restTemplate.getForObject(urlC, Map.class);

                if (result != null) {

                    List<Map<String, Object>> castMovies =
                            (List<Map<String, Object>>) result.get("cast");

                    if (castMovies != null) {
                        List<Map<String, Object>> filtered =
                                castMovies.stream()
                                        .filter(m -> "movie".equals(m.get("media_type")) || m.get("title") != null)
                                        .filter(m -> m.get("release_date") != null)
                                        .filter(m -> ((String) m.get("release_date")).length() >= 4)
                                        .toList();

                        allMovies.addAll(filtered);
                    }
                }
            }
        }

        Map<Integer, Map<String, Object>> uniqueMovies = new HashMap<>();

        for (Map<String, Object> movie : allMovies) {
            Integer id = (Integer) movie.get("id");
            uniqueMovies.put(id, movie);
        }

        List<Map<String, Object>> mergedMovies =
                new ArrayList<>(uniqueMovies.values());

        // 🔹 Ranking filmów
        List<Map<String, Object>> rankedMovies = mergedMovies.stream().map(movie -> {
                    int score = 0;
